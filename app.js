...
function renderNotifications() {
  ...
  if (getSessionItem(sessionNotificationsClearedKey) === "true") {
    notificationPanel.hidden = false;
    notificationBoard.innerHTML = '<p class="task-empty">Notifications cleared.</p>';
    return;
  }
    ...
    meetings.forEach((meeting) => {
      ...
      if (eventDate && dayBeforeEvent === todayManilaDate) {
        notifications.unshift(`
          <article class="attendance-item is-highlight attendance-item-reminder">
            <p class="attendance-name">Event Reminder</p>
            <p class="attendance-time">${meeting.title} is scheduled tomorrow.</p>
            <p class="attendance-note">${meeting.date} at ${meeting.time}${meeting.place ? ` • ${meeting.place}` : ""}</p>
          </article>
        `);
      }
      ...
      if (eventDate && eventDate === todayManilaDate && isSixAmOrLaterInManila) {
        notifications.unshift(`
          <article class="attendance-item is-highlight attendance-item-reminder">
            <p class="attendance-name">Today's Event Reminder</p>
            <p class="attendance-time">${meeting.title} is scheduled today.</p>
            <p class="attendance-note">${meeting.date} at ${meeting.time}${meeting.place ? ` • ${meeting.place}` : ""}</p>
          </article>
        `);
      }
  ...
  if (currentStaffName === headOfPassengerServiceName) {
    if (attendanceLog.length) {
      ...
      notifications.push('<p class="task-empty">No staff login records yet.</p>');
    }
  }
  ...
  if (currentStaffName === "Wendixter Cutanda") {
    if (isMondayInManila && isSixAmOrLaterInManila) {
      notifications.unshift(`
        <article class="attendance-item is-highlight attendance-item-reminder">
          <p class="attendance-name">Monday 6:00 AM Reminder</p>
          <p class="attendance-time">Please pass the Compliance B and C to the Collector's Office today.</p>
          <p class="attendance-note">This reminder is shown only on Wendixter Cutanda's account every Monday starting at 6:00 AM.</p>
        </article>
      `);
    }
    ...
    if (isFridayInManila) {
      notifications.unshift(`
      <article class="attendance-item is-highlight attendance-item-reminder">
        <p class="attendance-name">Friday Reminder</p>
        <p class="attendance-time">Please update the BOC Currency Exchange today.</p>
        <p class="attendance-note">This reminder is shown only on Wendixter Cutanda's account every Friday.</p>
      </article>
      `);
    }
    ...
    if (isFridayInManila && isSixAmOrLaterInManila) {
      notifications.unshift(`
        <article class="attendance-item is-highlight attendance-item-reminder">
          <p class="attendance-name">Friday 6:00 AM Reminder</p>
          <p class="attendance-time">Please pass the BWC Report to the Collector's Office today.</p>
          <p class="attendance-note">This reminder is shown only on Wendixter Cutanda's account every Friday starting at 6:00 AM.</p>
        </article>
      `);
    }
    ...
    if (latestDutyEntry && latestDutyEntry.staffName !== "Wendixter Cutanda") {
      notifications.unshift(`
        <article class="attendance-item is-highlight">
          <p class="attendance-name">Current Duty Staff: ${latestDutyEntry.staffName}</p>
          <p class="attendance-time">${latestDutyEntry.timestamp}</p>
          <p class="attendance-note">Most recent login recorded for today.</p>
        </article>
      `);
    }
    ...
    if (otherStaffLogins.length) {
      notifications.push(...otherStaffLogins.map((entry, index) => `
        <article class="attendance-item ${index === 0 ? "is-highlight" : ""}">
          <p class="attendance-name">${entry.staffName}</p>
          <p class="attendance-time">${entry.timestamp}</p>
          <p class="attendance-note">${index === 0 ? "Latest staff login for today." : "Logged in today."}</p>
        </article>
      `));
    } else {
      notifications.push('<p class="task-empty">No other staff have logged in today yet.</p>');
    }
  }
  ...
  if (currentStaffName === "Unfernie Pulvera" && isMondayInManila) {
    notifications.unshift(`
      <article class="attendance-item is-highlight attendance-item-reminder">
        <p class="attendance-name">Monday Reminder</p>
        <p class="attendance-time">Please pass the Compliance A or the FCD Reporting to the Collector's Office today.</p>
        <p class="attendance-note">This reminder is shown only on Unfernie Pulvera's account every Monday.</p>
      </article>
    `);
  }
  ...
  if (currentStaffName === "John Benedict Andaya" && isJohnDailyReminderTime) {
    notifications.unshift(`
      <article class="attendance-item is-highlight attendance-item-reminder">
        <p class="attendance-name">6:00 PM Reminder</p>
        <p class="attendance-time">Please report the Daily Turnover Report to the Collector's Office.</p>
        <p class="attendance-note">This reminder is shown only on John Benedict Andaya's account every day starting at 6:00 PM.</p>
      </article>
    `);
  }

  notificationPanel.hidden = false;
  notificationBoard.innerHTML = notifications.length
    ? notifications.join("")
